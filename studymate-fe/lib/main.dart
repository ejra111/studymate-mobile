...
class _StudyMateAppState extends State<StudyMateApp> {
  ...
  @override
  void dispose() {
    controller.latestNotification.dispose();
    controller.dispose();
    super.dispose();
  }
...
class AppController extends ChangeNotifier {
  ...
  void _handleNewNotification(Map<String, dynamic>? data) {
    if (data == null || !data.containsKey('notification')) return;

    final notif = asMap(data['notification']);
  ...
  void _handleStudyInvite(Map<String, dynamic>? data) {
    if (data == null || !data.containsKey('notification')) return;
  ...
  void _handlePrivateMessage(Map<String, dynamic>? data) {
    ...
    // We'll handle this more in chat screen later
  ...
  void _showNotificationPopup(Map<String, dynamic> notif) {
    ...
    Future.delayed(const Duration(seconds: 5), () {
      if (latestNotification.value == notif) {
  ...
  Future<bool> uploadAvatar(XFile file) async {
    final hasFace = await hasExactlyOneFace(file);
    if (!hasFace) {
      lastError = 'Foto profil harus memiliki tepat 1 wajah.';
      notifyListeners();
      return false;
    }
...
class _ProfileTabState extends State<ProfileTab> {
  ...
  void didUpdateWidget(ProfileTab oldWidget) {
    ...
    // Check if user data changed and refill form
    fill();
    setState(() {});
  }
